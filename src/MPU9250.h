...
class MPU9250
{
  ...
  private:
    TwoWire * i2c_bus;						// Allows for use of various I2C ports

 	  SPIClass* spi_bus;						// Allows for use of different SPI ports
  ...
  public:
    ...
    float temperature; // internal chip temperature in Celsius

    float ax, ay, az, gx, gy, gz; // latest sensor data values
    float mx = -1;
    float my = -1;
    float mz = -1;

    float pitch, yaw, roll; // fused sensor values
    ...
    bool checkWhoAmI();
    bool checkWhoAmIAK8963();

    void readMPU9250Data(int16_t * data);
    ...
    void initAK8963Slave();
    void initMPU9250();
    void calibrateMPU9250(int32_t * gyroBias, int32_t * accelBias);
    void setAccGyroBias(const int32_t* gyroBias, const int32_t* accelBias);
    void MPU9250SelfTest(float * destination);
    void magCalMPU9250(float * bias, float * scale);
    void setMagBiasScale(const float bias[3], const float scale[3]);
    void resetMPU9250();
